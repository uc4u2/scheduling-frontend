...
const resolveCanonical = (explicitCanonical) => {
  if (explicitCanonical) return explicitCanonical;
  try {
    if (typeof window !== "undefined") {
      const { origin, pathname, search } = window.location || {};
      if (origin) return `${origin}${pathname || ""}${search || ""}`.replace(/\/+$/, pathname ? "" : "/");
    }
  } catch {
    /* noop */
  }
...
const Meta = ({ title, description, canonical, og = {}, twitter = {} }) => {
  useEffect(() => {
    ...
    const twitterPayload = {
      ...
      title: ogPayload.title || title || previousTitle,
      description: resolvedDescription,
    ...
    twitterEntries.forEach(([key, value]) => {
      setMetaTag("name", `twitter:${key}`, value);
    });
