...
const buildInvoiceEmailTemplateOptions = ({ row, tInvoice, customTemplates = [] }) => {
  const clientName = String(row?.client_name || row?.client?.name || row?.client?.display_name || "").trim() || "there";
  const companyName = String(row?.invoice_display_business_name || row?.company_name || "Schedulaa").trim() || "Schedulaa";
  const invoiceNumber = row?.invoice_number || "your invoice";
  const remainingBalance = formatCurrency(row?.remaining_balance || row?.total || 0, row?.currency || "USD");
  const builtIns = [
    {
...
export default function FinanceInvoicesPage({ onNavigate }) {
  ...
  const emailTemplateOptions = useMemo(
    () => buildInvoiceEmailTemplateOptions({ row: emailTarget, tInvoice, customTemplates: emailTemplates }),
    [emailTarget, emailTemplates, tInvoice]
  );
  ...
  const openSendEmailDialog = useCallback((row) => {
    ...
    if (defaultCustom) {
      ...
      setEmailTemplateKey("payment_reminder");
      const builtIn = buildInvoiceEmailTemplateOptions({ row, tInvoice, customTemplates: [] }).find((entry) => entry.key === "payment_reminder");
      setEmailSubject(builtIn?.subject || `Invoice ${row?.invoice_number || ""}`.trim());
  ...
  useEffect(() => {
    ...
    loadEmailTemplates();
  }, [emailClientId, emailDialogOpen, loadEmailDocuments, loadEmailTemplates]);
  ...
  const handleDeleteEmailAttachment = async (row) => {
    if (!emailClientId || !row?.id) return;
    const value = String(row.id);
    setEmailDocumentDeletingId(value);
    try {
      await deleteManagerClient360Document(emailClientId, row.id);
      await loadEmailDocuments(emailClientId);
      setEmailDocumentIds((prev) => prev.filter((item) => item !== value));
      enqueueSnackbar(tInvoice("snackbar.attachmentRemoved", "Attachment removed."), { variant: "success" });
    } catch (err) {
      enqueueSnackbar(
        err?.response?.data?.error ||
          err?.message ||
          tInvoice("errors.removeAttachment", "Unable to remove the attachment."),
        { variant: "error" }
      );
    } finally {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      await sendFinanceInvoiceEmail(invoiceId, {
        ...
        subject: String(emailSubject || "").trim() || undefined,
  ...
  const renderInvoiceCards = (rows) => (
    <Stack spacing={1.25}>
      {rows.map((row) => {
        ...
        const canUsePaymentLink = isInvoicePaymentLinkActionable({
          status: row?.payment_status || row?.status,
          remainingBalance: row?.remaining_balance,
        });
        ...
        const menuActions = [
          ...
          },
          {
            ...
            help: tInvoice("actionHelp.sendInvoice", "Email the invoice to the client with an optional PDF attachment and optional payment link."),
      ...
      <Dialog open={emailDialogOpen} onClose={emailSending ? undefined : () => setEmailDialogOpen(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers>
          <Stack spacing={2} sx={{ mt: 0.5 }}>
            ...
            <TextField
              label={tInvoice("emailDialog.subject", "Subject")}
              value={emailSubject}
            ...
            <TextField
              ...
              helperText={tInvoice("emailDialog.helperText", "The invoice summary is added automatically. Delivery options below control the PDF attachment and online payment link.")}
            />
            <Stack spacing={1} sx={{ pt: 0.5 }}>
              <Typography variant="subtitle2">
                {tInvoice("emailDialog.deliveryOptionsTitle", "Delivery options")}
              </Typography>
              <Box>
                <label style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <input
                    type="checkbox"
                    checked={emailAttachInvoicePdf}
                    onChange={(event) => setEmailAttachInvoicePdf(event.target.checked)}
                  />
                  <span>{tInvoice("emailDialog.attachPdfLabel", "Attach invoice PDF")}</span>
                </label>
                <Typography variant="caption" color="text.secondary">
                  {tInvoice("emailDialog.attachPdfHelp", "Attach a formal PDF copy for the client’s records.")}
                </Typography>
              </Box>
              <Box>
                <label style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <input
                    type="checkbox"
                    checked={emailIncludePaymentLink}
                    disabled={Boolean(emailPaymentLinkDisabledReason)}
                    onChange={(event) => setEmailIncludePaymentLink(event.target.checked)}
                  />
                  <span>{tInvoice("emailDialog.paymentLinkLabel", "Include online payment link")}</span>
                </label>
                <Typography variant="caption" color="text.secondary">
