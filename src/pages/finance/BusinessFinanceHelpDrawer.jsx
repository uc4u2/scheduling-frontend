...
const COPY = {
  ...
  fa: {
    ...
    shortItems: [
      "درخواست قیمت = مشتری قیمت سفارشی می‌خواهد",
      "برآورد = قیمت پیشنهادی برای مشتری",
      "فاکتور = سابقه پرداخت",
  ...
  ru: {
    ...
    shortItems: [
      "Запрос = клиент просит индивидуальную цену",
      "Смета = предложение цены для клиента",
...
COPY.fa.workflowSections = COPY.en.workflowSections.map((entry, index) => {
  const translations = [
    {
      title: "۱. با یک درخواست قیمت شروع کنید",
      body: [
    ...
    {
      title: "۴. برآورد را به اشتراک بگذارید",
      body: [
    ...
    {
      title: "۵. مشتری صفحه عمومی برآورد را بررسی می‌کند",
      body: [
    ...
    {
      title: "۶. وضعیت‌های برآورد را بشناسید",
      body: [
    ...
    {
      title: "۸. اگر مشتری رد کند یا پاسخ ندهد",
      body: [
    ...
    {
      title: "۹. فاکتور، پرداخت و دستور کار",
      body: [
    ...
    {
      title: "۱۰. کار میدانی کارمند و تأیید مدیر",
      body: [
    ...
    {
      title: "۱۱. گزارش‌گیری و پایان ماه",
      body: [
...
COPY.zh.workflowSections = COPY.en.workflowSections.map((entry, index) => {
  const translations = [
    { title: "1. 从报价请求开始", body: ["当客户需要定制定价且你还没有准备报价单时，请使用报价请求。", "例如：厨房漏水维修、柜体维修、定制安装，或任何价格取决于工作范围的项目。", "不要把这个流程用于已知价格的简单预约。如果服务、价格、员工和时间都已明确，请改用普通 Booking 流程。"], bullets: ["标题", "来源", "请求联系人", "服务地址", "期望时间", "描述", "可见备注", "内部备注"], footer: "报价请求是起始记录，不是报价单、发票或工单。" },
    ...
    { title: "3. 创建报价单", body: ["当你准备好面向客户的价格方案时，创建报价单。", "分享前请检查客户、报价标题、签发日期、到期日期、备注、条款、税额和明细项。", "如果该请求已经生成报价单，请打开现有报价单，而不是再建一个。"], bullets: ["报价标题", "签发日期", "到期日期", "可见备注", "条款", "明细项", "税额和折扣"], footer: "报价单就是这项工作的对客报价。" },
    { title: "4. 分享报价单", body: ["最佳操作是发送报价单。这样会把公开报价链接发送给客户。", "如果需要手动分享，也可以使用创建/复制链接、打开链接、打印/PDF 或复制摘要。", "WhatsApp、短信和个人邮箱目前仍是手动方式，除非以后增加自动化。"], footer: "手动标记为已发送只是内部状态捷径，不能证明客户已收到或已批准报价单。" },
    { title: "5. 客户查看公开报价页面", body: ["公开报价页面会显示公司名称、报价编号、标题、明细项、总额、备注和条款。", "它不会显示内部备注、盈利能力、人工成本、供应商成本、审计日志或仅供财务使用的数据。", "客户可以在公开页面上批准或拒绝报价。"], footer: "如果客户批准，报价单会变成 approved；如果拒绝，则变成 rejected。" },
    { title: "6. 理解报价状态", body: ["Draft 表示报价单仍在准备中。", "Sent 表示报价单已经分享或已通过邮件发送。", "Viewed 表示客户打开了公开链接。", "Approved 表示客户已接受，或经理作为管理补救手段手动标记为已接受。", "Rejected 表示客户已拒绝，或经理作为管理补救手段手动标记为已拒绝。"], footer: "经理手动批准或拒绝只是补救状态，不等同于客户通过公开链接给出的真实回应。" },
    { title: "7. 如果客户接受", body: ["批准后，由经理决定下一步。", "转为发票会创建计费和收款记录。", "创建工单会生成运营执行记录。", "这些动作应保持分开，因为报价批准、收款和工作执行是不同的业务步骤。"], bullets: ["转为发票", "创建 / 复制付款链接", "打开付款链接", "创建工单", "分配团队"], footer: "常见路径：客户批准 -> 发票和付款链接 -> 工单 -> 分配 -> 现场报告 -> 经理审核。" },
    { title: "8. 如果客户拒绝或未回复", body: ["如果客户拒绝，请查看备注，修改报价，并在合适时使用 Revise and Resend。", "如果客户没有回应，请跟进。沉默绝不能视为批准。", "如果报价单已经转为发票，不要重新打开它。应通过发票处理变更，或创建新的修订报价单。"], footer: "Viewed 但无回应表示需要跟进。Expired 表示要决定是否延长并重新发送。" },
    { title: "9. 发票、付款和工单", body: ["当客户需要付款时，转为发票。", "如果希望客户在线支付，请使用创建/复制付款链接或打开付款链接。", "当工作准备好排期和执行时，创建工单。", "工单是工作执行计划，并不等同于报价单或发票。"], bullets: ["工作日期", "地点", "说明", "计划材料", "已分配团队成员"], footer: "计划材料暂时不会扣减库存。" },
    { title: "10. 员工现场工作和经理批准", body: ["员工可以看到已分配的工单并提交现场报告。", "现场报告记录现场发生了什么，但不会单独更新库存或发票记录。", "经理复核现场报告，并决定哪些内容成为正式记录。", "只有经理审核批准后，库存才会发生变化。"], bullets: ["员工提交现场报告", "经理打开 Field Reports", "经理创建 Review", "经理批准材料和计费决定", "当已批准的审核完成工作时，工单关闭"], footer: "员工报告实际情况，经理批准哪些内容成为正式记录。" },
    { title: "11. 报表和月末", body: ["使用 Profitability 查看收入、人工、已批准材料成本、关联费用和预计利润。", "Tax Summary 只供会计复核使用。它是估算值，不是正式报税。", "在检查缺失项并准备导出给会计时，使用 Month-End 和 Reports。"], footer: "报表用于复核和导出，不替代正式会计或税务申报。" },
  ];
  return translations[index] || entry;
});
