...
export default function ExpenseQuickAddDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || err?.message || "Unable to save expense.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={open} onClose={loading ? undefined : onClose} fullWidth maxWidth="md">
      ...
      <DialogContent dividers>
        <Stack spacing={2.5} sx={{ mt: 0.5 }}>
          ...
          <Alert severity="info">
            Receipt file upload will be added later. For now, paste a receipt link or attach metadata.
          </Alert>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={3}>
              <TextField fullWidth label="Tax" type="number" inputProps={{ step: "0.01" }} value={form.tax_amount} onChange={(e) => setField("tax_amount", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={6}>
            ...
            <Grid item xs={12} md={6}>
              <TextField fullWidth label="Vendor" value={form.vendor_name} onChange={(e) => setField("vendor_name", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                <InputLabel>Linked client</InputLabel>
                <Select label="Linked client" value={form.client_id} onChange={(e) => setField("client_id", e.target.value)}>
            ...
            <Grid item xs={12} md={6}>
              <TextField fullWidth label="Linked entity type" value={form.linked_entity_type} onChange={(e) => setField("linked_entity_type", e.target.value)} />
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField fullWidth label="Linked entity ID" type="number" value={form.linked_entity_id} onChange={(e) => setField("linked_entity_id", e.target.value)} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth label="Receipt links or notes" value={form.receipt_text} onChange={(e) => setField("receipt_text", e.target.value)} multiline minRows={3} helperText="Use one link or note per line." />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth label="Note" value={form.note} onChange={(e) => setField("note", e.target.value)} multiline minRows={3} />
            </Grid>
            <Grid item xs={12}>
              <FormControlLabel
                control={<Switch checked={form.is_recurring_template} onChange={(e) => setField("is_recurring_template", e.target.checked)} />}
                label="Mark as recurring expense template"
              />
            </Grid>
          </Grid>
        </Stack>
