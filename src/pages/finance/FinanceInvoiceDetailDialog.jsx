...
export default function FinanceInvoiceDetailDialog({
  ...
  const [sendEmailIncludeReviewCta, setSendEmailIncludeReviewCta] = useState(false);
  const [sendEmailReviewCapability, setSendEmailReviewCapability] = useState(null);
  const [sendEmailReviewCapabilityLoading, setSendEmailReviewCapabilityLoading] = useState(false);
  ...
  const selectedEmailTemplate = useMemo(
    () => emailTemplateOptions.find((entry) => entry.key === sendEmailTemplateKey) || null,
  ...
  const handleOpenEmailTemplateManager = useCallback((mode = "create", template = null) => {
    if (mode === "delete" && template?.template_id) {
      ...
      deleteManagerClient360EmailTemplate(template.template_id)
        .then(async () => {
          ...
          if (sendEmailTemplateKey === `custom:${template.template_id}`) {
            setSendEmailTemplateKey("");
          }
        })
        .catch((err) => {
    ...
    if (mode === "default" && template?.template_id) {
      ...
      setManagerClient360EmailTemplateDefault(template.template_id)
        .then(async (payload) => {
          ...
          await loadEmailTemplates();
          if (saved?.id) {
            setSendEmailTemplateKey(`custom:${saved.id}`);
          }
        })
        .catch((err) => {
    ...
    setEmailTemplateDraft(
      template || {
        name: sendEmailSubject || "Payment link email",
  ...
  }, [enqueueSnackbar, loadEmailTemplates, selectedEmailTemplate, sendEmailMessage, sendEmailSubject, sendEmailTemplateKey]);
  ...
  const handleSaveEmailTemplate = async (overrideForm = null) => {
    ...
    try {
      ...
      if (saved?.id) {
        setSendEmailTemplateKey(`custom:${saved.id}`);
        setSendEmailSubject(payload.subject);
        setSendEmailMessage(payload.body);
      }
    } catch (err) {
            ...
            <Stack spacing={1} sx={{ pt: 0.5 }}>
              ...
              <Box>
                <label style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <input
                    type="checkbox"
                    checked={sendEmailIncludeReviewCta}
                    disabled={sendEmailReviewCapabilityLoading || !sendEmailReviewCapability?.eligible}
                    onChange={(event) => setSendEmailIncludeReviewCta(event.target.checked)}
                  />
                  <span>{tDetail("emailDialog.reviewCtaLabel", "Include Google review request")}</span>
                </label>
                <Typography variant="caption" color="text.secondary">
                  {getInvoiceReviewCtaHelperText(
                    sendEmailReviewCapabilityLoading ? null : sendEmailReviewCapability
                  )}
                </Typography>
              </Box>
            </Stack>
