...
const SavedPayrollsPortal = ({ token, currentUser }) => {
  ...
  // NEW – departments & recruiters
  const [departments, setDepartments] = useState([]);          // ⇠ step-1
  const [departmentFilter, setDepartmentFilter] = useState(""); // ⇠ step-1
  ...
  useEffect(() => {
    ...
    if (isManager) {
      fetchDepartments();           // ⇠ step-1
      fetchRecruiters();            // will auto-filter if departmentFilter set
    }
    fetchPayrolls();
  ...
  return (
    <ManagementFrame title="Saved Payrolls" subtitle="Search, email or export saved payroll runs.">
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 3, flexWrap: "wrap" }}>
        ...
        {isManager && (
          <>
            {/* department dropdown – step-3 */}
            <Select
              value={departmentFilter}
              onChange={(e) => {
                setDepartmentFilter(e.target.value);
                setRecruiterFilter(""); // reset recruiter when dept changes
              }}
              displayEmpty
              size="small"
            >
            ...
            >
              ...
              {recruiters.map((r) => (
                <MenuItem key={r.id} value={r.id}>
                  {r.first_name} {r.last_name} {/* step-4 fix */}
                </MenuItem>
              ))}
            </Select>
          </>
        ...
        <Button onClick={fetchPayrolls} variant="outlined">
          Apply Filters
        </Button>
      </Box>
        ...
        <Box sx={{ display: "flex", alignItems: "center", mb: 2, gap: 1 }}>
          ...
          {selected.length > 0 && (
            <>
              ...
              <Button onClick={handleBulkDownload} variant="outlined">
                Download Selected
              </Button>
              <Button
        ...
        <TableBody>
          {payrolls.map((p) => (
            <TableRow key={p.id}>
              ...
              <TableCell>
                <Button onClick={() => handleDownload(p.id)} size="small">
                  Download
                </Button>
                <Button
                  variant="outlined"
                ...
                <Tooltip title="Manually Issue">
                  <IconButton
                    onClick={() => handleManualIssue(p.id)}
                    size="small"
                    sx={{ ml: 1 }}
                  >
                    <UploadFileIcon />
