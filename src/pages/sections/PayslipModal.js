  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent dividers>
                  ...
                  ({ label, key }) =>
                    isNonZero(row[key]) && (
                      <TableRow key={key}>
                        ...
                        <TableCell>{money(row[key])}</TableCell>
                      </TableRow>
                    )
                )}
              </TableBody>
        ...
        {/*  --- Parental Leave block: add just before deductions --- */}
        {row.on_parental_leave && (
          <>
            <Divider sx={{ my: 2 }} />
            <Typography sx={{ fontWeight: 500 }}>Parental Leave</Typography>
            <Table size="small" sx={{ mb: 2 }}>
              <TableBody>
                <TableRow>
                  <TableCell>On Parental Leave</TableCell>
                  <TableCell>Yes</TableCell>
                </TableRow>
                {isNonZero(row.parental_leave_hours) && (
                  <TableRow>
                    <TableCell>Leave Hours</TableCell>
                    <TableCell>{row.parental_leave_hours}</TableCell>
                  </TableRow>
                )}
                {isNonZero(row.parental_top_up) && (
                  <TableRow>
                    <TableCell>Employer Top-up</TableCell>
                    <TableCell>{money(row.parental_top_up)}</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
