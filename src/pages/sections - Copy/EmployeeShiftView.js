...
const EmployeeShiftView = () => {
  ...
  const fetchAllRecruiters = async () => {
    try {
      ...
      // 🧠 Auto-select first recruiter if manager and nothing is selected
      if (
        userRole === "manager" &&
        data.recruiters?.length > 0 &&
        selectedRecruiters.length === 0
      ) {
        setSelectedRecruiters([String(data.recruiters[0].id)]);
      }
    } catch {
  ...
  const exportToExcel = () => {
    ...
    saveAs(new Blob([buf], { type: "application/octet-stream" }), `Shifts_${recruiter.name || "employee"}.xlsx`);
  };
...
/>
      ...
      <Typography variant="h4" gutterBottom>
        📋 {recruiter.name || "Employee"} — Shift Overview
      </Typography>
...
{userRole === "manager" && (
  ...
  <Typography variant="subtitle2" color="text.secondary" gutterBottom>
    For: {allRecruiters.find(r => String(r.id) === String(currentRecruiterId))?.name || "Unknown"}
  </Typography>
)}
