...
content = textwrap.dedent("""import React, { useMemo } from \"react\";
import {
  Box,
  Typography,
  Stack,
...
const AboutPage = () => {
  ...
  const heroTitle = getArray(heroData.title, DEFAULT_CONTENT.hero.title);
  const heroBadgeContent = heroData.badge || DEFAULT_CONTENT.hero.badge;
  const heroPrimaryCta = heroData.primaryCta || DEFAULT_CONTENT.hero.primaryCta;
  const heroSecondaryCta = heroData.secondaryCta || DEFAULT_CONTENT.hero.secondaryCta;
  const heroMediaAlt = heroData.mediaAlt || DEFAULT_CONTENT.hero.mediaAlt;
  ...
  const valueCards = useMemo(
    () =>
      introValues.map((value, index) => {
        const key = value.key || DEFAULT_CONTENT.intro.values[index]?.key;
        const IconComponent = (key && VALUE_ICON_MAP[key]) || AutoAwesomeIcon;
        return {
          title: value.title || DEFAULT_CONTENT.intro.values[index]?.title || "",
          description:
            value.description ||
            DEFAULT_CONTENT.intro.values[index]?.description ||
            "",
    ...
    () => (
      <Stack direction=\"row\" spacing={1.5} alignItems=\"center\">\n        <Box\n          sx={{\n            width: 48,\n            height: 48,\n            borderRadius: 14,\n            background: `linear-gradient(135deg, ${alpha(
        ...
        <Stack spacing={0.25}>
          ...
          >
            {heroBadgeContent?.overline || DEFAULT_CONTENT.hero.badge.overline}
          </Typography>
          <Typography
            variant=\"subtitle2\"
            sx={{ fontWeight: 600, color: theme.palette.text.secondary }}
          >
            {heroBadgeContent?.title || DEFAULT_CONTENT.hero.badge.title}
          </Typography>
        </Stack>
      </Stack>
    ),
    [heroBadgeContent, theme]
  );
  ...
  return (
    <Box sx={{ position: \"relative\", overflow: \"hidden\" }}>
      ...
      <HeroShowcase
        eyebrow={heroData?.eyebrow || DEFAULT_CONTENT.hero.eyebrow}
        title={heroTitle}
        subtitle={heroData?.subtitle || DEFAULT_CONTENT.hero.subtitle}
        primaryCTA={{
          label: heroPrimaryCta?.label || DEFAULT_CONTENT.hero.primaryCta.label,
          to: heroPrimaryCta?.to || DEFAULT_CONTENT.hero.primaryCta.to,
          variant: \"contained\",
        }}
        secondaryCTA={{
          label: heroSecondaryCta?.label || DEFAULT_CONTENT.hero.secondaryCta.label,
          to: heroSecondaryCta?.to || DEFAULT_CONTENT.hero.secondaryCta.to,
          variant: \"outlined\",
        }}
...
Path('src/landing/pages/AboutPage.js').write_text(content, encoding='utf-8')
